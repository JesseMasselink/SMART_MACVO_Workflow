...
class MACVONode(Node):
    def __init__(
        ...
        #start time for timestamp header
    ...
    def publish_data(self, system: MACVO):
        ...
        time = Time()
        time_ns_total = self.start_time.nanoseconds + time_ns
        print("time_ns:", time_ns)
        time.sec = time_ns_total // 1_000_000_000
        print("time_s", time.sec)
        time.nanosec = time_ns_total % 1_000_000_000
        pose_msg = to_stamped_pose(pose, self.coord_frame, time)
        
        # Latest map
        if system.mapping:
            points = system.graph.get_frame2map(system.graph.frames[:])
        else:
